...
6. **Enforces** PR policy in GitHub CI (failing closed with exit code 1 on `BLOCK` and `INSUFFICIENT_EVIDENCE`), gates external mutations behind human engineer approval, and **writes the investigation outcome back into DataHub via MCP `save_document` and `add_tags` tools**.
...
| **1:20–1:45** | **Impact Graph & BLOCK Decision** | "Sentinel constructs the blast-radius graph and returns BLOCK with 94% Evidence Coverage. It isolates confirmed executive dashboard and ML model consumers from unaffected billing assets." |
...
| **2:05–2:25** | **GitHub Gate & DataHub Writeback** | "GitHub Actions CI blocks the PR with exit code 1. Upon human approval, clicking 'Persist to DataHub' executes MCP `save_document` and `add_tags`, saving an investigation document into DataHub." |
...
- **Code Evidence**: [`apps/api/app/datahub/mcp_client.py`](file:///c:/Users/mouav/OneDrive/Desktop/sentinelai/apps/api/app/datahub/mcp_client.py) & [`apps/api/app/datahub/client.py`](file:///c:/Users/mouav/OneDrive/Desktop/sentinelai/apps/api/app/datahub/client.py).
...
- **Implementation**: FastAPI backend, Next.js 14 frontend, SQLGlot AST engine, and SQLAlchemy persistence.
...
- **Testing**: 100% passing pytest test suite (`pytest tests/`) and Next.js production build verification (`npm run build`).
